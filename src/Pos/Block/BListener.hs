...
data BListener m = BListener
    { onApplyBlocks :: forall ssc. SscHelpersClass ssc =>
                                       OldestFirst NE (Blund ssc) -> m ()
    , onRollbackBlocks :: forall ssc. SscHelpersClass ssc =>
                                          NewestFirst NE (Blund ssc) -> m ()
    }

bListenerStub :: Monad m => BListener m
bListenerStub =
    BListener {onApplyBlocks = const pass, onRollbackBlocks = const pass}
